...
class FPABlock(nn.Module):
    def __init__(
        ...
        self.down1 = nn.Sequential(
            ...
            ConvBnRelu(in_channels, 1, kernel_size=7, stride=1, padding=3)
        )
    ...
    def forward(self, x):
        ...
        x2 = self.conv2(x2)
        x = x2 + x3
        x = F.interpolate(x, size=(d//2, h // 2, w // 2), **upscale_parameters)
...
class GAUBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),
            ConvBnRelu(out_channels, out_channels, kernel_size=1, add_relu=False),
            nn.Sigmoid()
        )
        self.conv2 = ConvBnRelu(in_channels, out_channels, kernel_size=3, padding=1)
...
if __name__ == '__main__':
    ...
    net = PAN(decoder_channels=32, activation='sigmoid')
    out = net(inputs)
    print(out.size())
