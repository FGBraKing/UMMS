...
class ParallelPolarizedSelfAttention(nn.Module):
    ...
    def __init__(self, channel=512):
        ...
        self.ch_wq=nn.Conv3d(channel,1,kernel_size=(1,1))
        ...
        self.agp=nn.AdaptiveAvgPool3d((1,1))
...
class SequentialPolarizedSelfAttention(nn.Module):
    ...
    def __init__(self, channel=512):
        ...
        self.sp_wv=nn.Conv3d(channel,channel//2,kernel_size=1)
        self.sp_wq=nn.Conv3d(channel,channel//2,kernel_size=1)
        self.agp=nn.AdaptiveAvgPool3d(1)

    def forward(self, x):
        b, c, d, h, w = x.size()

        # Channel-only Self-Attention
        channel_wv=self.ch_wv(x) #bs,c//2,d,h,w
        channel_wq=self.ch_wq(x) #bs,1,d,h,w
        channel_wv=channel_wv.reshape(b,c//2,-1) #bs,c//2,d*h*w
        channel_wq=channel_wq.reshape(b,-1,1) #bs,d*h*w,1
        channel_wq=self.softmax_channel(channel_wq)
        ...
        # Spatial-only Self-Attention
        spatial_wv=self.sp_wv(channel_out) # bs,c//2, d,h,w
        spatial_wq=self.sp_wq(channel_out) #bs,c//2,d,h,w
        spatial_wq=self.agp(spatial_wq) #bs,c//2,1,1,1
        spatial_wv=spatial_wv.reshape(b,c//2,-1) #bs,c//2,d,d*h*w
        spatial_wq=spatial_wq.permute(0,2,3,4,1).reshape(b,1,c//2) #bs,1,c//2
        spatial_wq=self.softmax_spatial(spatial_wq)
        spatial_wz=torch.matmul(spatial_wq,spatial_wv) #bs,1,h*w
        spatial_weight=self.sigmoid(spatial_wz.reshape(b,1,d,h,w)) #bs,1,h,w
