...
class ChilopodUnetWithFuseAndDeepsupervised(nn.Module):
    def __init__(self, in_channels, out_channels, domains=('source', 'target'),
        ...
        super(ChilopodUnetWithFuseAndDeepsupervised, self).__init__()
        ...
        self.outconvs = nn.ModuleList([OutConv(f_maps[i], out_channels, upsampling=2**i, with_activation=with_activation, final_sigmoid=final_sigmoid) for i in range(num_levels-2, -1, -1)])
    ...
    def forward(self, source, target, domains=('source', 'target')):
        ...
        if self.training:
            source_out = []
            target_out = []
            for decoder, fuser, source_encoder_features, target_encoder_features, out_conv in zip(self.decoders, self.fusers, source_encoders_features, target_encoders_features, self.outconvs):
                ...
                source_out.append(out_conv(source))
                target_out.append(out_conv(target))
            ...
            source = self.outconvs[-1](source)
            target = self.outconvs[-1](target)
...
if __name__ == "__main__":
    ...
    net = ChilopodUnetWithFuseAndDeepsupervised(1, 1, ['target', 'source'], DoubleConv,
