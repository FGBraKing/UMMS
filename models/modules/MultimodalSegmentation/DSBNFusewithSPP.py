...
class SPP(nn.Module):
    ...
    def __init__(self, c1, c2, k=(5, 9, 13), norm_type="batch", act_type="lrelu"):
        super().__init__()
        c_ = c1 // 2  # hidden channels

        self.conv1 = nn.Sequential(
            conv1x1x1(c1, c_),
            get_normalization3d(c_, norm_type),
            get_activation(act_type)
        )
        self.conv2 = nn.Sequential(
            conv1x1x1(c_ * (len(k) + 1), c2),
            get_normalization3d(c2, norm_type),
            get_activation(act_type)
        )
...
class ASPPConv(nn.Sequential):
    def __init__(self, in_channels, out_channels, dilation, norm_type="batch", act_type="lrelu", **norm_kwargs):
        use_bias = not (norm_type == 'batch' or norm_type == 'group')
        super().__init__(
            same_convlution(in_channels, out_channels, kernel_size=3, dilation=dilation, use_bias=use_bias),
            get_normalization3d(out_channels, norm_type, **norm_kwargs),
            get_activation(act_type),
        )


class ASPPPooling(nn.Sequential):
    def __init__(self, in_channels, out_channels, norm_type="batch", act_type="lrelu", **norm_kwargs):
        use_bias = not (norm_type == 'batch' or norm_type == 'group')
        super().__init__(
            nn.AdaptiveAvgPool3d(1),
            same_convlution(in_channels, out_channels, kernel_size=1, use_bias=use_bias),
            get_normalization3d(out_channels, norm_type, **norm_kwargs),
            get_activation(act_type),
        )
...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_type="batch", act_type="lrelu", **norm_kwargs):
        ...
        modules.append(ASPPConv(in_channels, out_channels, rate1, norm_type, act_type, **norm_kwargs))
        modules.append(ASPPConv(in_channels, out_channels, rate2, norm_type, act_type, **norm_kwargs))
        modules.append(ASPPConv(in_channels, out_channels, rate3, norm_type, act_type, **norm_kwargs))
        modules.append(ASPPPooling(in_channels, out_channels, norm_type, act_type, **norm_kwargs))

        self.convs = nn.ModuleList(modules)

        self.project = nn.Sequential(
            same_convlution(5 * out_channels, out_channels, kernel_size=1, use_bias=use_bias),
            get_normalization3d(out_channels, norm_type, **norm_kwargs),
            get_activation(act_type),
            # nn.Dropout(0.5),
        )
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, domains, kernel_size=3,
                 norm_type="batch", act_type="lrelu", dilation=1, use_norm=True, use_act=True, **norm_kwargs):
        super(ConvBlock, self).__init__()
        use_bias = not (norm_type == 'batch' or norm_type == 'group')
        self.conv = same_convlution(in_channels, out_channels, kernel_size, dilation=dilation, use_bias=use_bias)
...
class ChilopodUnetFuseWithSPP(nn.Module):
    def __init__(self, in_channels, out_channels, domains=('source', 'target'),
                 basic_module=DoubleConv, f_maps=16, num_levels=5, with_spp=True,
        ...
        super(ChilopodUnetFuseWithSPP, self).__init__()
        ...
        self.decoder_refiners = nn.ModuleList([OutConv(f_maps[i], f_maps[0], upsampling=2 ** i, with_activation=False)
                                               for i in range(num_levels - 2, -1, -1)])

        if with_spp:
            self.pp = SPP(f_maps[0]*4, f_maps[0], (5, 9, 13))
        else:
            self.pp = ASPP(f_maps[0]*4, f_maps[0], (6, 12, 18))

        self.outconv = OutConv(f_maps[0], out_channels, with_activation=with_activation, final_sigmoid=final_sigmoid)

    def get_encoders_features(self, x, domain):
        encoders_features = []
        for encoder in self.encoders:
            x = encoder(x, domain)
            encoders_features.insert(0, x)
        encoders_features = encoders_features[1:]
        return encoders_features, x

    def forward_nouse(self, source, target, domains=('source', 'target')):
        source_encoders_features, source = self.get_encoders_features(source, domains[0])
        target_encoders_features, target = self.get_encoders_features(target, domains[1])

        source_decoders_features = []
        target_decoders_features = []
        for decoder, fuser, source_encoder_features, target_encoder_features, decoder_refiner in zip(self.decoders, self.fusers, source_encoders_features, target_encoders_features, self.decoder_refiners):
            ...
            source_decoders_features.append(decoder_refiner(source))
            target_decoders_features.append(decoder_refiner(target))

        source = self.pp(torch.cat(source_decoders_features, dim=1))
        target = self.pp(torch.cat(target_decoders_features, dim=1))
...
if __name__ == "__main__":
    ...
    net = ChilopodUnetFuseWithSPP(1, 1, ['target', 'source'], DoubleConv, with_spp=False,
    ...
    with torch.no_grad():
        s_in = torch.rand((2, 1, 80, 112, 112), requires_grad=True).to(device)  # 64,96,96
        t_in = torch.rand((2, 1, 80, 112, 112), requires_grad=True).to(device)  # 64,96,96
