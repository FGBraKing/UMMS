...
class UpBlock(nn.Module):
    def __init__(self, in_channels, feature_channels, fuse_channels, out_channels,
        ...
        self.basic_module = basic_module(in_channels + feature_channels + fuse_channels, out_channels,
                                         False, conv_kernel_size, norm_type, act_type)

    def forward(self, encoder_features, x, fuse_features):
...
def create_decoders(f_maps, basic_module, conv_kernel_size, interpolation, norm_type, act_type):
    ...
    for i in range(len(reversed_f_maps) - 1):
        ...
        decoder = UpBlock(in_feature_num, cat_feature_num, fuse_feature_num, out_feature_num, conv_kernel_size,
...
class SingleUnetWithFuse(nn.Module):
    def __init__(self, in_channels, out_channels, domains=None, basic_module=DoubleConv, f_maps=16, num_levels=5,
        ...
        super(SingleUnetWithFuse, self).__init__()
    ...
    def forward(self, source, target, domain=None):
        ...
        for decoder, fuser, source_encoder_features, target_encoder_features in zip(self.decoders, self.fusers,
                                                                                    source_encoders_features,
                                                                                    target_encoders_features):
            fuse_features = fuser(source, target)
            source = decoder(source_encoder_features, source, fuse_features)
            target = decoder(target_encoder_features, target, fuse_features)
...
if __name__ == "__main__":
    ...
    net = SingleUnetWithFuse(1, 1, ['target', 'source'], DoubleConv,
    ...
    source = torch.rand((1, 1, 80, 112, 112), requires_grad=True).to(device)  # 64,96,96
    target = torch.rand((1, 1, 80, 112, 112), requires_grad=True).to(device)  # 64,96,96
    with torch.no_grad():
        s_out, t_out = net(source, target)
