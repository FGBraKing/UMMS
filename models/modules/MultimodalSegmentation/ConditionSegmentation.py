...
def create_decoders(f_maps, basic_module, conv_kernel_size, interpolation, norm_type, act_type):
    ...
    for i in range(len(reversed_f_maps) - 1):
        ...
        cat_feature_num = reversed_f_maps[i + 1] + 1
        out_feature_num = reversed_f_maps[i + 1]

        # if i == 0:
        #     in_feature_num = in_feature_num + 1
...
class SingleUnet(nn.Module):
    ...
    def forward(self, x, posi_ratio, domain=None):
        encoders_features = []
        ...
        # prior = (torch.rand(x.size(0), 1, *x.size()[2:], device=x.device) < posi_ratio).float()
        #
        # x = torch.cat([x, prior], dim=1)
        # x = self.decoders[0](encoders_features[0], x)

        for decoder, encoder_features in zip(self.decoders, encoders_features):
            prior = (torch.rand(encoder_features.size(0), 1, *encoder_features.size()[2:],
                                device=encoder_features.device) < posi_ratio).float()
            encoder_features = torch.cat([encoder_features, prior], dim=1)
...
if __name__ == "__main__":
    ...
    net = SingleUnet(1, 1, ['target', 'source'], DoubleConv, 16, 5,
    ...
    # print(net)
    out = net(inputs, 0.12, 'source')
    print(out.size())
    print('--------------------------single model-------------------------------')
    single_out = net(inputs, 0.12)
    print(single_out.size())
    print_model_parm_flops(net, inputs, posi_ratio=0.12, need_idx=False)  # 751.84G
    print(net)
