...
SUPPORT_SCHEDULERS = ['cosine', 'tanh', 'poly', 'step', 'multistep', 'linear', 'plateau']
...
if __name__ == '__main__':
    ...
    fig.suptitle('scheduler', fontsize=14, fontweight='bold')

    for i in range(2):
        for j in range(3):
            sche = SUPPORT_SCHEDULERS[i*3+j]
            args = dict(
                lr_noise=None,
                lr_noise_pct=0.67,
                lr_noise_std=1.,
            ...
            try:
                ...
                print(type(e), e)
                continue
            ax[i, j].plot(range(1, num_epochs+1), data_list)
            ax[i, j].set_title(sche)
            # ax[i, j].legend()
