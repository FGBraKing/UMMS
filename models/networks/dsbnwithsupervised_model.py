...
from models.modules.MultimodalSegmentation.DomainSpecificNormalization import ChilopodUnet, BinaryKDLoss, CSALoss, FeatureMapExtractor
...
def define_model(opt, device, domains=None):
    ...
    net = ChilopodUnet(in_channels=opt.input_nc,
...
class DsbnWithSupervisedModel(BaseModel):
    def __init__(self, opt):
        super(DsbnWithSupervisedModel, self).__init__(opt)
        ...
        self.loss_names = ['regular', 'routine', 'total', 'kd', 'csa']  #

        self.feature_extractor = FeatureMapExtractor(self.net_umms, ['decoders.0.basic_module.conv2',
                                                                     'decoders.1.basic_module.conv2'])
        # 'decoders.1.upsampling', 'decoders.1.basic_module.conv2'
        if self.isTrain:
            ...
            self.criterionCSA = CSALoss(reduction='mean')   # "sum" 'mean'
        ...
        self.source_feature_k = None
        self.source_feature_l = None
        self.target_feature_k = None
        self.target_feature_l = None
    ...
    def forward(self):
        ...
        with self.autocast_context():
            ...
            self.source_feature_k, self.source_feature_l = self.feature_extractor.get_out_feature()
            self.target_predict = self.net_umms(self.target_volume, 'target')
            self.target_feature_k, self.target_feature_l = self.feature_extractor.get_out_feature()
        self.is_activated = False

    def forward_one(self, domain):
        if domain == "target":
            with self.autocast_context():
                self.target_predict = self.net_umms(self.target_volume, 'target')
                self.target_feature_k, self.target_feature_l = self.feature_extractor.get_out_feature()
    ...
    def backward(self):
        with self.autocast_context():
            ...
            if self.opt.network_type.lower() == "dsbnwithkd":
                with torch.no_grad():
                    self.loss_csa = self.criterionCSA(self.source_feature_k, self.source_feature_l, self.source_label,
                                                      self.target_feature_k, self.target_feature_l, self.target_label)
                self.loss_total = self.loss_routine + 1e-4 * self.loss_regular + 1.0 * self.loss_kd
            elif self.opt.network_type.lower() == "dsbnwithcsa":
                self.loss_csa = self.criterionCSA(self.source_feature_k, self.source_feature_l, self.source_label,
                                                  self.target_feature_k, self.target_feature_l, self.target_label)
                self.loss_total = self.loss_routine + 1e-4 * self.loss_regular + 1.0 * self.loss_csa
            else:
                self.loss_csa = 0
...
def main():
    ...
    model = DsbnWithSupervisedModel(opt)
