...
class DsbnWithEdgeModel(BaseModel):
    def __init__(self, opt):
        super(DsbnWithEdgeModel, self).__init__(opt)
        ...
        self.loss_names = ['regular', 'routine', 'edge', 'total']
    ...
    def backward(self):
        with self.autocast_context():
            ...
            # self.source_sampleweight, self.target_sampleweight
            self.loss_total = self.loss_routine + 1e-4*self.loss_regular + 1*self.loss_edge*1e-4
    ...
    def backward_one(self, domain):
        if domain == "target":
            predict = self.target_predict
            label = self.target_label
            pedge = self.target_pedge
            dismap = self.target_dismap
        else:
            predict = self.source_predict
            label = self.source_label
            pedge = self.source_pedge
            dismap = self.source_dismap

        with self.autocast_context():
            self.loss_regular = self.criterionRegular(self.net_umms.parameters())
            self.loss_edge = self.criterionEdge(pedge, dismap) * 1e4
            self.loss_item_dict, self.loss_routine = self.criterionRoutine(predict, label)
            self.loss_total = self.loss_routine + 1e-4*self.loss_regular + self.loss_edge*1e-4
        self.loss_total = self.loss_total / self.opt.gradient_accumulation_k_step
    ...
    def optimize_parameters(self, update=True):
        if self.opt.optimize_respective:
            self.optimize_parameters_respective(update)
        else:
            self.optimize_parameters_merge(update)
    ...
    def optimize_parameters_merge(self, update=True):
        if update:
            self.forward()
            self.backward()
            self.optimizer_step()
    ...
    def optimize_parameters_respective(self, update=True):
        if update:
            ...
            with self.no_sync_context():
                warnings.WarningMessage("you are alternating using source and target, "
                                        "and may make some fault when stop updating")
                self.forward_one("source")
                self.backward_one("source")
                source_loss_dict = self.record_loss_item("source")
                self.forward_one("target")
    ...
    def compute_metrics_base(self, predict, label, domain, *args, **kwargs):
        if self.net_umms.training:
            metric_names = ['DC', 'recall', 'precision', 'ravd', 'roisize']
        else:
            metric_names = ['DC', 'recall', 'precision', 'specificity', 'accuracy', 'hd', 'hd95', 'asd', 'assd', 'ravd', 'roisize']
        ...
        metrics = self.get_metrics_soft(predict, label, *metric_names,
    ...
    def get_current_metrics(self):
        ...
        metrics_ret['softravd'] = self.source_predict.sum() - self.target_predict.sum()
        return metrics_ret
