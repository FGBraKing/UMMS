...
from models.modules.MultimodalSegmentation.ConditionSegmentation import SingleUnet
...
class ConditionModel(BaseModel):
    def __init__(self, opt):
        super(ConditionModel, self).__init__(opt)
        ...
        self.loss_names = ['regular', 'combo', 'prior', 'total']
        if self.isTrain:
    ...
    def set_input(self, inputs):
        ...
        self.label_ratio = self.label.detach().sum()/self.label.detach().numel()

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        with self.autocast_context():
            self.predict = self.net_segment(self.volume, self.label_ratio)
        self.is_activated = False
    ...
    def backward(self):
        with self.autocast_context():
            ...
            self.loss_prior = self.criterionPrior(self.predict, self.label)
            self.loss_total = self.loss_combo + 1e-4*self.loss_regular + self.prior_gamma_base*self.loss_prior
