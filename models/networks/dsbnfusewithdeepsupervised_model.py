...
from models.modules.MultimodalSegmentation.DomainSpecificNormalization import FeatureMapExtractor
...
class DsbnFuseWithDeepSupervisedModel(BaseModel):
    def __init__(self, opt):
        super(DsbnFuseWithDeepSupervisedModel, self).__init__(opt)
        ...
        self.loss_names = ['regular', 'feature', 'routine', 'prior', 'total']  # 'kd',

        if self.isTrain:
            self.feature_extractor = FeatureMapExtractor(self.net_umms, ['decoders.0',
                                                                         'decoders.1',
                                                                         'decoders.2',
                                                                         'decoders.3'])
            ...
            self.criterionFeature = get_loss_criterion(name='prior_feature', use_mixed_precision=opt.use_mixed_precision)
        ...
        self.source_predicts = None
        self.target_predicts = None
        self.predict_weights = (0.4, 0.6, 0.8, 1.0)

        self.decoders_features = None
    ...
    def forward(self):
        ...
        with self.autocast_context():
            self.source_predicts, self.target_predicts = self.net_umms(self.source_volume, self.target_volume)
            self.source_predict = self.source_predicts[-1]
            self.target_predict = self.target_predicts[-1]
            self.decoders_features = self.feature_extractor.get_out_feature()
        self.is_activated = False

    def backward(self):
        with self.autocast_context():
            self.loss_regular = 1e-4 * self.criterionRegular(self.net_umms.parameters())
            self.loss_feature = 100 * self.criterionFeature(*tuple(zip(self.decoders_features[0::2],
                                                                       self.decoders_features[1::2])))
            self.loss_routine = 0
            self.loss_prior = 0
            for source_predict, target_predict, predict_weight in zip(self.source_predicts, self.target_predicts, self.predict_weights):
                self.loss_item_dict, loss_routine = self.criterionRoutineMulti(source_predict, self.source_label,
                                                                               target_predict, self.target_label)
                loss_prior = self.criterionPrior(source_predict, target_predict)
                if ex_config.current_epoch < self.opt.warmup_epochs:
                    loss_prior = 0 * loss_prior
                else:
                    loss_prior = self.prior_gamma_base * loss_prior
                self.loss_routine += predict_weight * loss_routine
                self.loss_prior += predict_weight * loss_prior

            self.loss_total = self.loss_routine + self.loss_regular + self.loss_prior
