...
from data.transforms.transformOnTensor import tensor2im, tensor2seg
...
class APADA2SEGModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_B = define_G(opt.output_nc, opt.input_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids)
        self.netS_B = define_S(opt.input_nc_seg, opt.output_nc_seg,
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netS_B.parameters(), self.netG_B.parameters()),
                                                lr=opt.lr, betas=(opt.optim_beta, 0.999))
            self.optimizer_D_A = torch.optim.Adam(self.netD_A.parameters(), lr=opt.lr, betas=(opt.optim_beta, 0.999))
            self.optimizer_D_B = torch.optim.Adam(self.netD_B.parameters(), lr=opt.lr, betas=(opt.optim_beta, 0.999))
    ...
    def get_current_visuals(self):
        ...
        seg_B = tensor2seg(torch.max(self.seg_fake_B.data, dim=1, keepdim=True)[1])
        manual_B = tensor2seg(torch.max(self.real_Seg.data, dim=1, keepdim=True)[1])
...
class APADA2SEGModel_TEST(BaseModel):
    ...
    def get_current_visuals(self):
        real_B = tensor2im(self.real_B.data)
        real_B_seg = tensor2seg(torch.max(self.real_B_seg.data, dim=1, keepdim=True)[1])
        gt_real_B_seg = tensor2seg(torch.max(self.gt_real_B_seg.data, dim=1, keepdim=True)[1])
