...
class StandardWithPriorModel(BaseModel):
    def __init__(self, opt, network):
        super(StandardWithPriorModel, self).__init__(opt)
        self.domains = ["source", "target"]

        self.model_names = ['umms']
        self.net_umms = network
        self.finally_activate = get_activation('sigmoid').to(self.device)

        self.loss_names = ['regular', 'routine', 'prior', 'total']

        if self.isTrain:
            self.criterionRoutine = get_loss_criterion(name='custom', use_mixed_precision=opt.use_mixed_precision)
