...
from models.modules.MultimodalSegmentation.DSBNFusewithSPP import ChilopodUnetFuseWithSPP
...
def define_model(opt, device, domains=None):
    ...
    net = ChilopodUnetFuseWithSPP(in_channels=opt.input_nc,
                                  out_channels=opt.output_nc,
                                  f_maps=opt.init_channel_number,
                                  num_levels=5,
                                  with_spp=opt.with_spp,
...
class DsbnFuseWithSPPModel(BaseModel):
    def __init__(self, opt):
        super(DsbnFuseWithSPPModel, self).__init__(opt)
        ...
        self.loss_names = ['regular', 'routine', 'feature', 'prior', 'total']  # 'kd',
    ...
    def backward(self):
        with self.autocast_context():
            ...
            if ex_config.current_epoch < self.opt.warmup_epochs:
                self.loss_prior = 0 * self.loss_prior
            else:
                self.loss_prior = self.prior_gamma_base * self.loss_prior
            # self.loss_prior = self.prior_gamma_base * self.loss_prior

            self.loss_feature = 10 * self.criterionFeature(*tuple(zip(self.decoders_features[0::2],
