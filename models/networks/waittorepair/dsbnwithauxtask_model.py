...
class DsbnWithAuxTaskModel(BaseModel):
    def __init__(self, opt):
        super(DsbnWithAuxTaskModel, self).__init__(opt)
        ...
        self.loss_names = ['regular', 'routine', 'regress', 'total']  # 'kd',
    ...
    def set_input(self, inputs):
        ...
        self.label_ratio = {'source': self.source_label.sum() / self.source_label.numel(),
                            'target': self.target_label.sum() / self.target_label.numel()}
    ...
    def backward(self):
        with self.autocast_context():
            ...
            if ex_config.current_epoch < 10:
                ...
                self.loss_regress = 10. * self.criterionRegress(self.ratio_predict, self.aim_ratio)
            if self.opt.network_type == 'finetunev1' or self.opt.network_type == 'finetunev2':
                ...
                self.loss_total = self.loss_regular + self.loss_regress + 1.0 * self.loss_routine + 0.6 * aux_routine1
            else:
                self.loss_item_dict, self.loss_routine = self.criterionRoutineMulti(self.source_predict, self.source_label,
                                                                                    self.target_predict, self.target_label)
                self.loss_total = self.loss_regular + self.loss_regress + self.loss_routine
