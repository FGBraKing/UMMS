...
def train():
    opt = get_opt(args=['--config_path=configs/defaults/adversarial_train.yaml', '--use_config', '--use_current_local_rank'])
    # dsbnwithauxtask_train
...
def do_train(opt):
    ...
    save_networks = model.save_networks
    ...
    for epoch in range(opt.epoch_start, opt.num_epochs + 1):
        ...
        for batch_idx, data in enumerate(dataloader, 1):
            ...
            if epoch < 11:
                ...
                optimize_parameters(batch_idx % 4 == 0)
