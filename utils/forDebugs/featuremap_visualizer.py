...
from utils.others.img_io import show_volume_label, show_array_3d, show_volume_label_predict
...
class FeatureMapExtractor:
    def __init__(self, model, layer_name):
        ...
        self.hook_the_model(model, layer_name)

    def feature_hooker(self, module, feature_in, feature_out):
        self.activations.append(feature_out)
    ...
    def hook_the_model(self, model, layer_name):
        for name, module in model.named_modules():
            if name == layer_name:
                self.handles.append(module.register_forward_hook(self.feature_hooker))
    ...
    def get_feature(self):
        assert self.activations, "nothing in the activations"
        return self.activations[-1].cpu().data.numpy()[0, ...]
...
def test_featuremap():
    kwargs = {
        ...
        'layer_name': 'encoders.1.double_conv.conv2.conv',
        'loss_name': 'custom',
    ...
    warp_network = FeatureMapExtractor(network, opt.layer_name)
    for patient_id, data in enumerate(test_dataloader):
        ...
        predict = warp_network(volume)

        featuremap = warp_network.get_feature()
        predict = predict.cpu().detach().numpy()[0, 0]
        volume = volume.cpu().numpy()[0, 0]
        label = label.cpu().numpy()[0, 0]

        print_numpy(featuremap, val=True, shp=True)
        print_numpy(predict, val=True, shp=True)
        print_numpy(volume, val=True, shp=True)
        print_numpy(label, val=True, shp=True)
        show_array_3d(featuremap[12], 3, 3)
