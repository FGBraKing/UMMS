...
# 先选样本，再选扩增方式。batch内样本不重叠
class MrusPlusDataset(NIIDataset):
    def __init__(self, opt):
        super(MrusPlusDataset, self).__init__(opt)
        self.mr_paths, self.us_paths = get_data_path(opt.dataroot, opt.phase, opt.fold)
        # 完全打乱需要使用不同的dataloader
        if opt.fake_shufflt:
        ...
        self.mr_index_generator = AugmentationIndex(opt.mirror_axes, opt.rot_axes,
                                                    shuffle=True, replacement=False, seed=24)
        ...
        self.data_size = min(self.us_size, self.mr_size)
    ...
    def get_aug_index(self, index):
        if isinstance(self.mr_aug_index[index], list) and len(self.mr_aug_index[index]) > 0:
            mr_aug_index = self.mr_aug_index[index].pop()
        else:
            self.mr_aug_index[index] = list(self.mr_index_generator)
            mr_aug_index = self.mr_aug_index[index].pop()
        if self.copy_aug:
            ...
            if isinstance(self.us_aug_index[index], list) and len(self.us_aug_index[index]) > 0:
                ...
                us_aug_index = self.us_aug_index[index].pop()
        return mr_aug_index, us_aug_index

    def parse_index(self, index):
        mirror_index = index // self.rotate_num
        rot_axis_index = (index % self.rotate_num) // 4
        rot_angle_index = ((index % self.rotate_num) % 4) // 1
        return mirror_index, rot_axis_index, rot_angle_index

    def apply_augmentation_args(self, data, mirror_aixs, rot_axis, rot_times):
...
# class PredictMrusPlusDataset(MrusPlusDataset):
#     def __init__(self, opt):
#         super(PredictMrusPlusDataset, self).__init__(opt)


class PredictMrusPlusDataset(NIIDataset):
    def __init__(self, opt):
        super(PredictMrusPlusDataset, self).__init__(opt)
        self.mr_paths, self.us_paths = get_data_path(opt.dataroot, opt.phase, opt.fold)
        self.mr_size = len(self.mr_paths)
